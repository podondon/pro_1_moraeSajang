    ...
    <section id="msgList">
        <div class="msgListInner">
            <div class="msgListInnerBox">
                <h2>보낸 쪽지</h2>
                <a href="sendAllDel" class="allDel" onclick="return del()">전체삭제</a>
                	<div class="noMsg">
                    <p>받은 쪽지가 없습니다.</p>
	...
	function drawMsgSendList(obj){
		...
		obj.sendList.forEach(function(item, idx){
			...
			if(item.messageContent.length === 20){
				content += '<div class="centerArea"><a href="sendDetail?messagesNo='+item.messagesNo+'"><p class="nickName">'+item.senerNickName+'</p><p class="msgContent">'+item.messageContent+'...</p></a></div>';
			}else{
				content += '<div class="centerArea"><a href="sendDetail?messagesNo='+item.messagesNo+'"><p class="nickName">'+item.senerNickName+'</p><p class="msgContent">'+item.messageContent+'</p></a></div>';
			}
			...
			content += '<a href="sendDel?messagesNo='+item.messagesNo+'" onclick="return del()"><svg width="22" height="22" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" class="delImg"><path fill="currentColor" d="M7 21q-.825 0-1.413-.588T5 19V6H4V4h5V3h6v1h5v2h-1v13q0 .825-.588 1.413T17 21H7ZM17 6H7v13h10V6ZM9 17h2V8H9v9Zm4 0h2V8h-2v9ZM7 6v13V6Z"/></svg></a>';
			content += '</div></li>';
		});
		$('.msgListBox').empty();
